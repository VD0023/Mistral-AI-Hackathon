...
## Draw labels for each menu segment directly on the ring.
@export var show_item_labels := true: set = _set_item_labels_display
## Font size used for labels drawn on each segment.
@export_range(8, 36, 1) var item_label_font_size := 12: set = _set_item_label_font_size
...
func _set_item_labels_display(new_display: bool):
	show_item_labels = new_display
	queue_redraw()

func _set_item_label_font_size(new_size):
	item_label_font_size = int(new_size)
	queue_redraw()
...
func _draw():
	...
	for i in range(count):			
		...
		if i == selected: 
			var selected_bg = _get_color("SelectedBackground")
			var selected_stroke = _get_color("SelectedStroke")
			if menu_items[i] is Dictionary:
				if menu_items[i].has("selected_bg_color") and menu_items[i]["selected_bg_color"] is Color:
					selected_bg = menu_items[i]["selected_bg_color"]
				if menu_items[i].has("selected_stroke_color") and menu_items[i]["selected_stroke_color"] is Color:
					selected_stroke = menu_items[i]["selected_stroke_color"]
			Draw.draw_ring_segment(self, coords, selected_bg, selected_stroke, sw, true)
		else:
			var bg = _get_color("Background")
			var stroke = _get_color("Stroke")
			if menu_items[i] is Dictionary:
				if menu_items[i].has("bg_color") and menu_items[i]["bg_color"] is Color:
					bg = menu_items[i]["bg_color"]
				if menu_items[i].has("stroke_color") and menu_items[i]["stroke_color"] is Color:
					stroke = menu_items[i]["stroke_color"]
			Draw.draw_ring_segment(self, coords, bg, stroke, sw, true)
...
func _draw_item_labels(start_angle: float):
	if not show_item_labels:
		return
	var n = menu_items.size()
	if n == 0:
		return
	...
	var font = _get_font("TitleFont")
	var color_default = _get_color("TitleDisplay")
	var fontsize = item_label_font_size

	var text_radius = get_icon_radius() + max(8.0, width * 0.12)
	var positions = Draw.calc_ring_segment_centers(
		text_radius,
		n,
		start_angle + item_angle * 0.5,
		start_angle + item_angle * 0.5 + n * item_angle,
		center_offset
	)
	...
	for i in range(n):
		...
		if not item.has("title"):
			continue
		var text = str(item["title"])
		if text.strip_edges() == "":
			continue

		var draw_color = color_default
		if item.has("label_color") and item["label_color"] is Color:
			draw_color = item["label_color"]
		if i == selected and item.has("selected_label_color") and item["selected_label_color"] is Color:
			draw_color = item["selected_label_color"]

		var size = font.get_string_size(text, HORIZONTAL_ALIGNMENT_CENTER, -1, fontsize)
		var pos = positions[i] - Vector2(size.x / 2.0, -font.get_descent())
		draw_string(font, pos, text, HORIZONTAL_ALIGNMENT_CENTER, -1, fontsize, draw_color)
